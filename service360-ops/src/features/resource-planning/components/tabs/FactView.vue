<template>
  <div class="normative-view">
    ...
    <h3 class="summary-title">Фактические ресурсы</h3>
    <p class="description">Средние фактические ресурсы по выбранной задаче</p>
    ...
    <div v-else class="resources-summary">
      ...
      <div v-if="factData.material && factData.material.length > 0" class="resource-section">
        ...
        <div class="resource-table">
          <div v-for="item in factData.material" :key="item.id" class="table-row">
      ...
      <div v-if="factData.personnel && factData.personnel.length > 0" class="resource-section">
        ...
        <div class="resource-table">
          <div v-for="item in factData.personnel" :key="item.id" class="table-row">
      ...
      <div v-if="factData.equipment && factData.equipment.length > 0" class="resource-section">
        ...
        <div class="resource-table">
          <div v-for="item in factData.equipment" :key="item.id" class="table-row">
      ...
      <div v-if="factData.tool && factData.tool.length > 0" class="resource-section">
        ...
        <div class="resource-table">
          <div v-for="item in factData.tool" :key="item.id" class="table-row">
      ...
      <div v-if="factData.service && factData.service.length > 0" class="resource-section">
        ...
        <div class="resource-table">
          <div v-for="item in factData.service" :key="item.id" class="table-row">
...
import { loadResourceAverage } from '@/shared/api/repairs/repairApi';
...
const props = defineProps({
  ...
  objLocationClsSection: { type: [Number, String], default: null },
...
const hasData = computed(() => {
  return (
    (factData.value.material && factData.value.material.length > 0) ||
    (factData.value.personnel && factData.value.personnel.length > 0) ||
    (factData.value.equipment && factData.value.equipment.length > 0) ||
    (factData.value.tool && factData.value.tool.length > 0) ||
    (factData.value.service && factData.value.service.length > 0)
  );
});

const fetchData = async () => {
  if (!selectedDate.value || !selectedPeriodType.value) return;
  loading.value = true;
  try {
    const result = await loadResourceAverage({
      objTask: props.objTask,
      Value: props.plannedVolume,
      objLocationClsSection: props.objLocationClsSection,
      objWork: props.objWork,
      periodType: selectedPeriodType.value.value,
      date: formatDateToISO(selectedDate.value),
    });
    factData.value = result[0] || {};
  } catch (error) {
    console.error('Ошибка загрузки фактических ресурсов:', error);
    notificationStore.showNotification('Не удалось загрузить фактические ресурсы.', 'error');
  } finally {
    loading.value = false;
  }
};

watch([selectedDate, selectedPeriodType], () => {
  fetchData();
});
...
onMounted(async () => {
  try {
    ...
    periodTypeOptions.value = types;
    const defaultType = types.find(t => t.value === 41);
    selectedPeriodType.value = defaultType || types[0] || null;
  } catch (error) {
